...
public class MyEnvelope implements Comparable, Serializable {
...
//Replace with direct comparisons. [Jon Aquino]
	 ...
	 */
	public MyEnvelope(MyEnvelope env) {
		init(env);
	}
	 ...
	 * @param env the Envelope to initialize from
	 */
	public void init(MyEnvelope env) {
		this.minx = env.minx;
		this.maxx = env.maxx;
		this.miny = env.miny;
		this.maxy = env.maxy;
...
// check for envelope disappearing
	 ...
	 */
	public void expandToInclude(MyEnvelope other) {
		if (other.isNull()) {
			return;
		}
		if (isNull()) {
	 ...
	 *         intersect
	 */
	public MyEnvelope intersection(MyEnvelope env) {
		...
		return new MyEnvelope(intMinX, intMaxX, intMinY, intMaxY);
	}
	 ...
	 */
	public boolean intersects(MyEnvelope other) {
	 ...
	 */
	public boolean disjoint(MyEnvelope other) {
		if (isNull() || other.isNull()) {
			return true;
		}
		return other.minx > maxx || other.maxx < minx || other.miny > maxy || other.maxy < miny;
	 ...
	 *             two-dimensional.
	 */
	public boolean overlaps(MyEnvelope other) {
	 ...
	 * @see #covers(Envelope)
	 */
	public boolean contains(MyEnvelope other) {
		return covers(other);
	}
	 ...
	 * <tt>contains</tt>, which would exclude the envelope boundary.
	 *
	 * @param p the point which this <code>Envelope</code> is being checked for
	 *          containing
	 ...
	 */
	public boolean covers(MyEnvelope other) {
		if (isNull() || other.isNull()) {
			return false;
		}
	 ...
	 */
	public double distance(MyEnvelope env) {
		if (intersects(env))
			return 0;
...
// if either is zero, the envelopes overlap either vertically or horizontally
	...
	public boolean equals(Object other) {
		if (!(other instanceof MyEnvelope)) {
			return false;
		}
		MyEnvelope otherEnvelope = (MyEnvelope) other;
		if (isNull()) {
